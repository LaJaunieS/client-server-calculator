 ...
 * attached Receiver subtracts operand1 from operand2, 
 * and result is then set from the result of the
 * computation
 * @author slajaunie
 *
 */
public class SubCommand extends AbstractCommand implements Serializable {
    private static final long serialVersionUID = -5507611038517172115L;

    /**Default constructor- sets operand1 and operand2 values to 
     * the <code>BigDecimal</code> value of 0, workMillisMin and
     ...
     * operands, workMillisMin and workMillisMax are both set 
     * to the given <code>double</code>, and result and 
     * receiver are set to <code>null</code>. . 
     ...
     * (in milliseconds) that the command will begin to execute
     */
     public SubCommand(BigDecimal operand1, BigDecimal operand2, 
     ...
     * (in milliseconds) that the command will begin to execute
     */
    public SubCommand(double operand1, double operand2,
